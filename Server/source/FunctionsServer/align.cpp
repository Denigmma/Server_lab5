...
std::vector<std::vector<Pixel>> loadImage(const char* filename, int& width, int& height) {
    ...
    if (!img_data) {
        std::cerr << "Error opening the file for align()" << std::endl;
        logMessage("Error opening the file for align()");
        exit(1);
    }
...
void align(const std::string& filename, const std::string& newfilename) {
    ...
    int width, height;
    std::vector<std::vector<Pixel>> image = loadImage(filepath.c_str(), width, height);
    float angle = findRotationAngle(image);

    std::string new_filepath2 = "../../../resources/" + newfilename;
    rotateImage(image, angle, new_filepath2.c_str());
    logMessage("align() successfully");
}